...
// Points at the Spring Boot backend in /backend. Change this if you deploy elsewhere.
const API_BASE = "http://localhost:8081/api/payments";
...
export default function PaymentCheckout() {
  ...
  const validate = () => {
    ...
    if (form.number.replace(/\D/g, "").length < 13) next.number = "Card number looks too short";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const intentRes = await fetch(`${API_BASE}/intents`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount, currency: "USD" }),
      });
      if (!intentRes.ok) throw new Error("Couldn't create payment intent");
      const intent = await intentRes.json();

      const confirmRes = await fetch(`${API_BASE}/intents/${intent.id}/confirm`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!confirmRes.ok || confirmed.status === "FAILED") {
        setApiError(typeof confirmed === "string" ? confirmed : confirmed.failureReason || "Payment failed");
        setStep("error");
        return;
      }
      ...
      setApiError("Couldn't reach the backend. Is it running on :8081?");
      setStep("error");
    }
  };
      ...
      <div style={{ maxWidth: 420, width: "100%" }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${LINE}`, borderRadius: 16 }} className="p-5 relative overflow-hidden">
          {step === "form" && (
            <form onSubmit={handleSubmit}>
              ...
              <p style={{ color: MUTED }} className="text-[11px] text-center mt-3">
                Demo card 4242 4242 4242 4242 succeeds · ending 0002 simulates a decline.
              </p>
            </form>
          )}
          ...
          {step === "success" && (
            <div className="flex flex-col items-center justify-center py-6">
              ...
              <p style={{ color: MUTED }} className="text-sm mt-1">
                ${amount.toFixed(2)} charged to {receipt?.cardBrand || cardBrand} ending in {receipt?.cardLast4 || form.number.slice(-4)}
              </p>
              <button
                onClick={reset}
          ...
          {step === "error" && (
            <div className="flex flex-col items-center justify-center py-6">
              <div
                style={{ width: 56, height: 56, borderRadius: "50%", background: "#FBEAE4" }}
                className="flex items-center justify-center"
              >
                <span style={{ color: CORAL }} className="text-2xl font-bold">
                  !
                </span>
              </div>
              <p style={{ color: INK }} className="text-base font-semibold mt-3">
              ...
              <p style={{ color: MUTED }} className="text-sm mt-1 text-center max-w-[280px]">
                {apiError}
              </p>
              <button
                onClick={() => setStep("form")}
                style={{ background: INK }}
                className="mt-5 text-white rounded-lg px-4 py-2 text-sm font-semibold"
              >
                Try again
              </button>
            </div>
